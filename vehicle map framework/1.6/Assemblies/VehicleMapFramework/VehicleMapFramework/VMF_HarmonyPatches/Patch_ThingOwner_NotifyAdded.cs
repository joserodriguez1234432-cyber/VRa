...
// Type: VehicleMapFramework.VMF_HarmonyPatches.Patch_ThingOwner_NotifyAdded
...
public static class Patch_ThingOwner_NotifyAdded
{
  public static void Postfix(Thing item, IThingHolder ___owner)
  {
    ...
    foreach (Thing thing in pawn.VehicleMap.listerBuildings.allBuildingsColonist.Where<Building>((Func<Building, bool>) (b => ThingCompUtility.HasComp<CompBuildableContainer>((Thing) b))))
      ThingCompUtility.TryGetComp<CompBuildableContainer>(thing).Notify_ThingAdded(item);
  }
}
