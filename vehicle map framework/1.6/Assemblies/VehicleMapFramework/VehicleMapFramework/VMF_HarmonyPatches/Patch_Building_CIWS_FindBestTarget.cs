// Decompiled with JetBrains decompiler
// Type: VehicleMapFramework.VMF_HarmonyPatches.Patch_Building_CIWS_FindBestTarget
// Assembly: VehicleMapFramework, Version=1.6.562.0, Culture=neutral, PublicKeyToken=null
// MVID: 10A61882-945F-4CFC-9B06-CA8EEF5ADB36
// Assembly location: D:\Programas\steamapps\workshop\content\294100\3426502333\1.6\Assemblies\VehicleMapFramework.dll

using HarmonyLib;
using System;
using System.Collections.Generic;
using System.Runtime.CompilerServices;
using Verse;

#nullable disable
namespace VehicleMapFramework.VMF_HarmonyPatches;

[HarmonyPatchCategory("VMF_Patches_IRBM")]
[HarmonyPatch]
public static class Patch_Building_CIWS_FindBestTarget
{
  [PatchLevel(Level.Safe)]
  public static void Postfix(Thing __instance, ref ILoadReferenceable __result)
  {
    if (__result != null)
      return;
    Map targetMap = TargetMapUtility.get_TargetMap(__instance);
    try
    {
      foreach (Map mapAndVehicleMap in __instance.Map.BaseMapAndVehicleMaps(false))
      {
        TargetMapUtility.set_TargetMap(__instance, mapAndVehicleMap);
        __result = Patch_Building_CIWS_FindBestTarget.FindBestTarget(__instance);
        if (__result != null)
          break;
      }
    }
    finally
    {
      TargetMapUtility.set_TargetMap(__instance, targetMap);
    }
  }

  [HarmonyReversePatch]
  [PatchLevel(Level.Mandatory)]
  [MethodImpl(MethodImplOptions.NoInlining)]
  public static ILoadReferenceable FindBestTarget(Thing instance)
  {
    Transpiler((IEnumerable<CodeInstruction>) null);
    throw new NotImplementedException();

    static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instructions)
    {
      return (IEnumerable<CodeInstruction>) instructions.MethodReplacer(MethodInfoCache.CachedMethodInfo.g_Thing_Map, MethodInfoCache.CachedMethodInfo.m_TargetMapOrThingMap);
    }
  }
}
